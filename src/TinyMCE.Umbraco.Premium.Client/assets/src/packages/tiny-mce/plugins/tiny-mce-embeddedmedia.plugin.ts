...
export default class UmbTinyMceEmbeddedMediaPlugin extends UmbTinyMcePremiumPluginBase {
	constructor(args: TinyMcePremiumPluginArguments) {
		super(args);
		const localize = new UmbLocalizationController(args.host);

		this.editor.ui.registry.addToggleButton('tinymceembeddialog', {
			icon: 'embed',
			tooltip: localize.term('general_embed'),
			onAction: () => this.#onAction(),
			onSetup: function (api) {
				const changed = args.editor.selection.selectorChangedWithUnbind('div.tiny-embed-holder', (state) =>
					api.setActive(state),
				);
	...
	#onAction() {
		...
		if (selectedElm.nodeName.toUpperCase() === 'DIV' && selectedElm.classList.contains('tiny-embed-holder')) {
	...
	#insertInEditor(embed: UmbEmbeddedMediaModalValue, activeElement: HTMLElement) {
		...
		const wrapper = this.editor.dom.create(
			'div',
			{
				class: 'mceNonEditable tiny-embed-holder',
		...
		if (activeElement?.nodeName.toUpperCase() === 'DIV' && activeElement.classList.contains('tiny-embed-holder')) {
