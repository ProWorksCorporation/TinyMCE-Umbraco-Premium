...
import { TINYMCE_MEDIA_CAPTION_ALT_TEXT_MODAL } from '../modals/media-caption-alt-text/media-caption-alt-text-modal.token.js';
//import { TINYMCE_MEDIA_CAPTION_ALT_TEXT_MODAL } from '../modals/media-caption-alt-text/media-caption-alt-text-modal.token.js';
...
export default class UmbTinyMceMediaPickerPlugin extends UmbTinyMcePremiumPluginBase {
	...
	constructor(args: TinyMcePremiumPluginArguments) {
		...
		this.editor.ui.registry.addToggleButton('tinymcemediapicker', {
			icon: 'image',
		...
		// but only if the tinymcemediapicker button is present
		const toolbar = this.configuration?.getValueByAlias<string[]>('toolbar');
		if (toolbar?.includes('tinymcemediapicker')) {
	...
	*/
	...
	async #showMediaCaptionAltText(mediaUnique: string | null) {
		...
		const modalHandler = this.#modalManager?.open(this, TINYMCE_MEDIA_CAPTION_ALT_TEXT_MODAL, { data: { mediaUnique } });
