...
@customElement('umb-input-tiny-mce-premium')
export class UmbInputTinyMcePremiumElement extends UUIFormControlMixin(UmbLitElement, '') {
	@property({ attribute: false })
	configuration?: UmbPropertyEditorConfigCollection;

	#plugins: Array<ClassConstructor<UmbTinyMcePremiumPluginBase> | undefined> = [];
	...
	async #setTinyConfig(additionalConfig?: RawEditorOptions) {
		...
		const editors = await renderEditor(config).catch((error) => {
			console.error('Failed to render TinyMCE Premium', error);
			return [];
		});
...
export default UmbInputTinyMcePremiumElement;

declare global {
	interface HTMLElementTagNameMap {
		'umb-input-tiny-mce-premium': UmbInputTinyMcePremiumElement;
	}
}
